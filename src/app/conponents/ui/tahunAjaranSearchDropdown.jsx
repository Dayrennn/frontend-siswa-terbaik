...
export default function TahunAjaranSearchDropdown({ value, onChange, initialLabel }) {
    const [search, setSearch] = useState(initialLabel || value || '');
    const [open, setOpen] = useState(false);
    const wrapperRef = useRef();

    const { data, isLoading } = useSeeAllTahunAjaranQuery();
    const tahunAjaranList = data?.data ?? [];

    const filtered = tahunAjaranList.filter(
        (k) =>
            k.namaTahunAjaran.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const handleSelect = (tahunAjaran) => {
        setSearch(`${tahunAjaran.namaTahunAjaran} - ${tahunAjaran.status}`);
        onChange(tahunAjaran.id);
        setOpen(false);
    };
                ...
                <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                    {isLoading ? (
                        ...
                        filtered.map((k) => (
                            ...
                            >
                                <span className="font-medium">{k.namaTahunAjaran}</span>
                                <span className="text-gray-400"> — {k.status}</span>
                            </button>
                        ))
                    )}
                </div>
