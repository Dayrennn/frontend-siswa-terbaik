...
import { useSeeAllGuruQuery } from '@/src/hooks/api/userSliceAPI';

export default function GuruSearchDropdown({ value, onChange, initialLabel }) {
    const [search, setSearch] = useState(initialLabel || value || '');
    const [open, setOpen] = useState(false);
    const wrapperRef = useRef();

    const { data, isLoading } = useSeeAllGuruQuery();
    const guruList = Array.isArray(data?.data) ? data.data : data?.data ? [data.data] : [];

    const filtered = guruList.filter((g) => g.username?.toLowerCase().includes(search.toLowerCase()));
    ...
    const handleSelect = (guru) => {
        setSearch(guru.username);
        onChange(guru.id);
        setOpen(false);
    };
