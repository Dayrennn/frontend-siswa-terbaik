...
import { useCreatePelajaranMutation } from '../../../../../hooks/api/pelajaranSliceAPI';

export default function FormTambahPelajaran({ onSuccess, onCancel }) {
    const [namaPelajaran, setNamaPelajaran] = useState('');
    const [kodePelajaran, setKodePelajaran] = useState('');

    const [createPelajaran, { isLoading, isError, error }] = useCreatePelajaranMutation();

    const handleCreate = async (e) => {
        e.preventDefault();

        try {
            const result = await createPelajaran({
                namaPelajaran,
                kodePelajaran,
            }).unwrap();

            // simpan sementara
            const namaPelajaranSuccess = namaPelajaran;
            const kodePelajaranSuccess = kodePelajaran;
            ...
            if (onSuccess) {
                onSuccess(result, {
                    namaPelajaran: namaPelajaranSuccess,
                    kodePelajaran: kodePelajaranSuccess,
                });
            }
        } catch (err) {
