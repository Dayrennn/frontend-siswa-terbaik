...
import { useCreateEskulMutation } from '@/src/hooks/api/eskulSliceAPI';

export default function FormTambahEskul({ onSuccess, onCancel }) {
    const [namaEskul, setNamaEskul] = useState('');

    const [createEskul, { isLoading, isError, error }] = useCreateEskulMutation();

    const handleCreate = async (e) => {
        e.preventDefault();

        try {
            const result = await createEskul({
                namaEskul,
            }).unwrap();
