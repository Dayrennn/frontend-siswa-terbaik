...
import { useModifyEskulMutation } from '@/src/hooks/api/eskulSliceAPI';

export default function FormEditDataEskul({ initialData, onSuccess, onCancel }) {
    const [namaEskul, setNamaEskul] = useState(initialData?.namaEskul || '');

    const [updateEskul, { isLoading, isError, error }] = useModifyEskulMutation();

    const handleEdit = async (e) => {
        e.preventDefault();

        try {
            const result = await updateEskul({ id: initialData.id, data: { namaEskul } }).unwrap();
    ...
    return (
        <form onSubmit={handleEdit} className="space-y-4">
            <div>
                <label className="text-sm text-gray-600">Nama Eskul</label>
                <input
                    type="text"
