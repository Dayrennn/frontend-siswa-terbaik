...
import { useGetUsersQuery, useRemoveUserMutation } from '../../../../hooks/api/userSliceAPI';
    ...
    const columns = [
            ...
            render: (row) => (
                <span className="text-gray-700">
                    {Array.isArray(row.pelajaran) && row.pelajaran.length > 0
                        ? row.pelajaran.map((p) => p.pelajaran?.namaPelajaran).join(', ')
                        : '-'}
                </span>
            ),
        },
        {
            ...
            render: (row) => (
                <span className="text-gray-700">
                    {Array.isArray(row.pelajaran) && row.pelajaran.length > 0
                        ? row.pelajaran.map((p) => p.pelajaran?.kodePelajaran).join(', ')
                        : '-'}
                </span>
            ),
        },
