...
import { useGetAllKriteriaQuery, useRemoveKriteriaMutation } from '../../../../hooks/api/kriteriaSliceAPI';
...
export default function DataKriteria() {
    ...
    const columns = [
        { key: 'no', label: 'No' },
        {
            key: 'namaKriteria',
            label: 'Nama Kriteria',
            render: (row) => <span className="text-gray-700">{row.namaKriteria || '-'}</span>,
        },
        {
            key: 'bobot',
            label: 'Bobot ',
            render: (row) => <span className="text-gray-700">{row.bobot || '-'}</span>,
        },
        {
            key: 'jenis',
            label: 'Jenis',
